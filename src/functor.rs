//! Provides higher order `Arbitrary` traits mainly for use by `proptest_derive`.
...
///
/// For these reasons, this trait is deliberatly not exported
/// in a convenient way.
pub trait ArbitraryF1<A: Debug>: Debug {
    //==========================================================================
...
/// `ArbitraryF2` lets you lift a [`Strategy`] to binary
...
///
/// See `ArbitraryF1`
pub trait ArbitraryF2<A: Debug, B: Debug>: Debug {
...
macro_rules! lift1 {
     ...
     $base: ident, $args: ident => $logic: expr) => {
        ...
        for $typ {
            ...
            fn lift1_with<S>($base: S, $args: Self::Parameters)
                -> $crate::proptest::strategy::BoxedStrategy<Self>
            where
                S: $crate::proptest::strategy::Strategy + 'static,
                S::Value: $crate::proptest::strategy::ValueTree<Value = A>
            {
                $crate::proptest::strategy::Strategy::boxed($logic)
            }
        }
    };
    ...
    ([$($bounds : tt)*] $typ: ty; $mapper: expr) => {
        lift1!(['static + $($bounds)*] $typ; base =>
            $crate::proptest::strategy::Strategy::prop_map(base, $mapper));
    };
    ([$($bounds : tt)*] $typ: ty) => {
        lift1!(['static + $($bounds)*] $typ;
            b => $crate::utils::from_map_strategy(b));
    };
}